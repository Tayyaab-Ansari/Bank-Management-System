...
public:
    ...
    virtual void report() const = 0;//The Account class is an abstract base class because it contains pure virtual functions. An abstract class cannot be instantiated on its own; it's meant to be a base for derived classes.
...
void CreateAccount::create_account() {
    ...
    while (type != 'C' && type != 'S') {
        ...
        cin >> type;
        type = toupper(type);// the toupper function is used to convert a character to its uppercase 
    }
...
greencolor();
    ...
    cin.ignore();
    cin.get();// Pause the program and wait for the user to press Enter
...
void CreateAccount::dep(double amount) {
	...
	if(num!=account_number)
	 {
        // Display an error message if the entered account number does not match
        redcolor();
        cout << "\n\n\t\t\t\t\t------------------------------";
        cout << "\n\t\t\t\t\t   Account number does not match!";
        cout << "\n\t\t\t\t\t------------------------------";
...
void CreateAccount::report() const {
	lightAqua();
    cout << "\t\t\t" << account_number << setw(10) << " " << name << setw(10) << " " << type << setw(6) << deposit << endl;//Using setw to set the field width to 10 characters
}
...
int main() {
    ...
    char ch;
    int num;
    CreateAccount *ac = NULL; // CreateAccount is a derived class from the Account class, and the use of a pointer allows for dynamic allocation of objects.
     intro();
    do {
        system("cls");
        if (ac == NULL) {
            ac = new CreateAccount(); // Creating a new account object using dynamic memory
        }
        ac->program_title();
       	bluecolor();
        cout << "\n\n\t\t\t\t\t----------------- MAIN MENU -----------------";
...
Aquacolor();	cout<<"\n\t\t\t\tCreated By : "; lightAqua();
	cout<<"\n\t\t\t\t\tATTA UL ASAD(FA22-BCS-073)\n";
	Aquacolor(); cout<<"\n\t\t\t\tSubject Name: ";lightAqua(); cout<<"OOP (Object Oriented language)";
	Aquacolor(); cout<<"\n\t\t\t\tUniversity: "; lightAqua(); cout<<" COMSATS University Islamabad, Vehari Campus.";
	cin.get();
	  system("cls");
}
//************************
